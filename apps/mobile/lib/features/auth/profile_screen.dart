...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _editSlotTime(String slot, String label) async {
    ...
    final duplicate = _slotTimes.entries.any((entry) {
      if (entry.key == slot) return false;
      final value = entry.value;
      return value[0] == picked.hour && value[1] == picked.minute;
    });
    ...
    if (duplicate) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('복약 알림 시간은 서로 겹치게 설정할 수 없습니다.'),
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
              '$label 알림이 ${picked.hour.toString().padLeft(2, '0')}:${picked.minute.toString().padLeft(2, '0')}로 변경되었습니다.'),
  ...
  Future<void> _showEditName() async {
    ...
    try {
      await ApiClient.patch('/users/me/name',
          token: _token, body: {'name': ctrl.text.trim()});
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content: Text('이름이 변경되었습니다.'), backgroundColor: Colors.green),
        );
      }
    } catch (e) {
      if (mounted) {
  ...
  Widget _slotTimeTile(String slot, String label) {
    ...
    final timeStr =
        '${times[0].toString().padLeft(2, '0')}:${times[1].toString().padLeft(2, '0')}';
