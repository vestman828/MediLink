...
class _AddMedicineScreenState extends State<AddMedicineScreen> {
  ...
  Future<void> _loadUser() async {
    ...
    final query = _medicineNameCtrl.text.trim();
    if (token != null && query.isNotEmpty) {
  ...
  Future<void> _searchMedicines(String query) async {
    ...
    try {
      ...
      _showSnack('약 검색에 실패했습니다. 서버 연결/인증 상태를 확인해주세요.', isError: true);
    }
  }
  ...
  Future<void> _save() async {
    ...
    if (dose.isEmpty) {
      _showSnack('1회 복용량을 입력해주세요.', isError: true);
      return;
    }
    ...
    try {
      final unitRaw = (_selectedMedicine?['unit'] ?? '정').toString().trim();
      final selectedUnit = unitRaw.isEmpty ? '정' : unitRaw;
      ...
      if (patientMedicineId == null) {
        throw const ApiException('복용 정보 생성에 실패했습니다.', 500);
      }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(
          widget.patientName == null ? '약 추가' : '${widget.patientName} 약 추가',
        ),
      ),
          ...
          children: [
            ...
            TextField(
              ...
              decoration: InputDecoration(
                hintText: '약 이름을 입력하면 검색됩니다. 예: 타이레놀',
                filled: true,
              ...
              Container(
                ...
                child: ListView.separated(
                  ...
                  itemBuilder: (_, i) {
                    ...
                    final unit = (medicine['unit'] ?? '정').toString();
                    return ListTile(
                      dense: true,
