...
class PatientHomeScreenState extends State<PatientHomeScreen> {
      ...
      if (mounted) {
        ...
        if (_schedules.isNotEmpty) {
          await NotificationService.scheduleFromSlots(
            _schedules
                .map((s) => {'time_slot': s['time_slot'] as String})
                .toList(),
          );
        }
      }
  ...
  Future<void> _checkIntake(int scheduleId) async {
    try {
      ...
      final slot = _schedules.firstWhere((s) => s['schedule_id'] == scheduleId,
          orElse: () => {});
      if (slot['time_slot'] != null) {
        await NotificationService.cancelReminderForSlot(
  ...
  Widget _buildNextDoseCard() {
    final slotTimes = {
      'morning': TimeOfDay(
          hour: _slotTimes['morning']?[0] ?? 8,
          minute: _slotTimes['morning']?[1] ?? 0),
      'lunch': TimeOfDay(
          hour: _slotTimes['lunch']?[0] ?? 12,
          minute: _slotTimes['lunch']?[1] ?? 0),
      'dinner': TimeOfDay(
          hour: _slotTimes['dinner']?[0] ?? 18,
          minute: _slotTimes['dinner']?[1] ?? 0),
      'bedtime': TimeOfDay(
          hour: _slotTimes['bedtime']?[0] ?? 22,
          minute: _slotTimes['bedtime']?[1] ?? 0),
    };
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          const SizedBox(width: 10),
          Text(
            '${slotLabels[nextSlot]} 복약까지 $timeStr',
            style: TextStyle(
  ...
  Widget _buildMemoButton() {
    return GestureDetector(
      onTap: () => Navigator.push(
        context,
        MaterialPageRoute(
            builder: (_) => DailyNoteScreen(date: DateTime.now())),
      ),
      child: Container(
        padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
        decoration: BoxDecoration(
  ...
  List<Widget> _buildScheduleList() {
    ...
    const slotOrder = ['morning', 'lunch', 'dinner', 'bedtime'];
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  '${schedule['dose']}  ${_displayTime(schedule['time_slot'] as String? ?? '', schedule['scheduled_time'] as String? ?? '')}',
                  style: const TextStyle(
  ...
  String _displayTime(String slot, String fallback) {
    final times = _slotTimes[slot];
    if (times == null) return fallback;
    final hh = times[0].toString().padLeft(2, '0');
    final mm = times[1].toString().padLeft(2, '0');
    return '$hh:$mm';
  }
}
