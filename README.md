...
- `infra/sql/007_medicines_data.sql` (deprecated)

예시(Windows + Docker):

```powershell
docker cp "infra\\sql\\001_init.sql" medilink-mysql:/001_init.sql
docker exec medilink-mysql mysql -u root -proot medilink -e "source /001_init.sql"
```
...
- `DRUG_API_KEY`: 공공데이터 의약품 API 사용 시 필요
